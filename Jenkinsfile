node {
    ...
    stage("SSH Docker Image Pull") {
        def dockerRun = 'sudo docker run --network cozubu-bridge -d -p 3000:3000 --name ${IMAGE_NAME} -v /app/node_modules -v /app:/app ${HARBOR_URL}/${HARBOR_PROJECT}/${IMAGE_NAME}:latest'
